...
def main():
    ...
    for i in range(q.shape[1]):
        ddx_r[:, i], dx_r[:, i], x_r[:, i], w_a[:, i], g = my_dmp.adapt(w[:, i], 
                x[0, i], arg.goal[i], t, s, psv, arg.samples, arg.rate) #arg.goal[i],x[-1, i]
        gain.append(g)
    ...
    if arg.show_plots:
        ...
        plot.expected_return(gain,save=False)
        plot.show_all()
