...
class obstacle(DMPs_cartesian):
    ...
    def __init__(self, x_obst = np.zeros(3), dx_obst = np.zeros(3),
            gamma_obst = 20.0, beta_obst = 10.0 / np.pi, beta_dist=1, **kwargs):
        """
    ...
    def gen_external_force(self, x, dx, goal):
        ...
        if np.linalg.norm(dx) > 1e-5:
            ...
            # print(vel)
            dist = np.linalg.norm(self.x_obst - x)
            # Calculate the steering angle
            phi = np.arccos(np.dot(pos, vel) /
                                    (np.linalg.norm(pos) * np.linalg.norm(vel)))

            dphi = self.gamma_obst * phi * np.exp(-self.beta_obst * phi-self.beta_dist*dist)
    ...
    def gen_ex(self, x, dx, goal):
        pval = 0. * self.x_obst
        # The repulsive force has to be computed only if we are moving
        if np.linalg.norm(dx) > 1e-5:
            dist = np.linalg.norm(self.x_obst - x)
        return dist
    # def xuanzhuan_force(self, dx1, dx2):
    #     r_xz = np.cross(dx1, dx2)
    #     R_xz = self.rotation_matrix(np.pi / 2, r_xz)
    #
    #     vel_xz = dx1
    #
    #     xz=np.dot(R_xz, vel_xz)
    #     return xz
